...
class Foxmetrics_Analytics_WooCommerce_Support {
	...
	public function foxmetrics_tracking_cart_add_item_callback(){
		...
		if ( !empty($_POST['product_id']) ) {
			...
			$product_category_name = (!empty($product_name_and_cat_name['product_category_name']) ? $product_name_and_cat_name['product_category_name'] : '');
			// prepare the script
			$result['event_script'] = "<!-- FoxMetrics Web Analytics Start --><script type='text/javascript'>_fxm.events.push(['_fxm.ecommerce.addcartitem','".$product_id."', '".$product_name."', '".$product_category_name."', '".$quantity."']);</script><!-- FoxMetrics Web Analytics End -->";
		}
		echo json_encode($result);
		wp_die();
	}
